...
(function() {
  var QueryBuilder, addToQuery, buildQuery, createUtils, expose, findOne, key, makeTest, multipleConditions, parseGetter, parseParamType, parseQuery, parseSubQuery, performQuery, performQuerySingle, queryFn, root, runQuery, score, single, testModelAttribute, testQueryValue, underscoreReplacement, utils, _i, _len, _ref,
    __slice = [].slice,
  ...
  queryFn = function(query, getter, isScore) {
    if (getter) {
      getter = parseGetter(getter);
    }
    if (!(utils.getType(query) === "Function")) {
      return single(parseQuery(query), getter, isScore);
    }
  };
